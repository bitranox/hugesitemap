...
- `src/hugesitemap/domain/filters.py` - Ordered wildcard/regexp drop-filter engine (Matcher, compile_filters, is_dropped)
...
### filters.py - Ordered drop-filter engine

Reproduces the original `sitemap_gen.py` filter behaviour: an ordered list of drop
patterns, each a shell wildcard (`fnmatch`) or a regexp prefixed with `re:`. A path
is dropped when any pattern matches. Patterns match against the path relative to its
root, normalised to forward slashes with a leading `/`.

| Symbol | Description |
|--------|-------------|
| `REGEX_PREFIX` | Marker (`re:`) that selects regexp matching |
| `Matcher` | A single compiled drop pattern (regexp or wildcard) |
| `compile_filters(patterns)` | Compile raw patterns into ordered `Matcher` objects |
| `is_dropped(relpath, matchers)` | Decide whether a relative path is dropped by any matcher |
...
| `GenerateRequest` | Full input for one site: `base_url`, `output_path`, `gzip`, `default_priority`, `directories`, `explicit_entries`, `drop_patterns`, `dry_run` |
...
- **Filters** use extend semantics: the global `[sitemap.filters].drop` patterns are
  prepended to each site's own `[site.filters].drop` (global first, then the site's).
