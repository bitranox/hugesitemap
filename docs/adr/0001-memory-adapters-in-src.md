...
interfaces (e.g. configuration loaders, content sources, sitemap writers) that avoid real
I/O. The question is whether these belong under `tests/` or under the
production source tree in `src/hugesitemap/adapters/memory/`.

## Decision

Place in-memory adapter implementations in
`src/hugesitemap/adapters/memory/` rather than in the test
tree.
