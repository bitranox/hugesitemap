...
from hugesitemap import __init__conf__, entry
...
def test_module_entry_executes_cli_and_shows_help(
    ...
    monkeypatch.setattr(sys, "argv", ["hugesitemap"], raising=False)

    with pytest.raises(SystemExit) as exc:
        runpy.run_module("hugesitemap.__main__", run_name="__main__")
...
def test_module_entry_formats_exceptions_via_exit_helpers(
    ...
    monkeypatch.setattr(sys, "argv", ["hugesitemap", "fail"], raising=False)
...
def test_module_entry_traceback_flag_prints_full_traceback(
    ...
    monkeypatch.setattr(sys, "argv", ["hugesitemap", "--traceback", "fail"])
    monkeypatch.setattr(lib_cli_exit_tools.config, "traceback", False, raising=False)
    monkeypatch.setattr(lib_cli_exit_tools.config, "traceback_force_color", False, raising=False)
...
def test_module_entry_subprocess_help() -> None:
    """Verify `python -m hugesitemap --help` works via subprocess.
    ...
    result = subprocess.run(
        [sys.executable, "-m", "hugesitemap", "--help"],
        capture_output=True,
        timeout=30,
        check=False,
        encoding="utf-8",
...
def test_module_entry_subprocess_version() -> None:
    """Verify `python -m hugesitemap --version` outputs version."""
    result = subprocess.run(
        [sys.executable, "-m", "hugesitemap", "--version"],
        capture_output=True,
        timeout=30,
        check=False,
        encoding="utf-8",
...
def test_entry_main_invokes_cli_with_help(
    ...
    (hugesitemap, hugesitemap).
    """
    monkeypatch.setattr(sys, "argv", ["hugesitemap", "--help"])
...
def test_entry_main_returns_nonzero_on_error(
    ...
    monkeypatch.setattr(sys, "argv", ["hugesitemap", "fail"])
